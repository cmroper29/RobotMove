...
executed), `--entry <routine>`, `--vendor`, `--strict` (exit 3 on warnings), `-q`.
...
All keys are optional. Relative paths are resolved against the config file.
...
**Common**
* Execution is static: IF/SWITCH/TEST take the first branch, WHILE/LOOP/REPEAT run once, and
  FOR loops with constant bounds are unrolled (loop variables work in expressions such as
  `path{i}`). Each simplification is reported as a warning. Subroutines are inlined.
...
* Control flow is not evaluated, and jumps (`JMP LBL`, `GOTO`) are not followed.
...
interpreters), `geometry.py` (lines, arcs, splines, blends), `kinematics.py` (pinocchio FK/IK),
`timing.py` (TOPP-RA), `planner.py` (dumb/smart), `output.py`, `cli.py`.
