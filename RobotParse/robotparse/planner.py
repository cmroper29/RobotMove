...
class _Resolver:
    ...
    # -- target helpers -----------------------------------------------------
    def cart_pose(self, target, m: Motion) -> Optional[np.ndarray]:
        where = m.source
        if isinstance(target, CartTarget):
            return m.base @ target.T
        if isinstance(target, JointTarget):
            if self.robot is None:
                self.diag.warn(f"{where}: joint target needs a URDF (--urdf) to compute the TCP; motion skipped")
                return None
        ...
        if isinstance(target, RelativeTarget):
            if target.joints is not None:
                if self.robot is None or self.q_cur is None:
                    self.diag.warn(f"{where}: relative joint move needs a URDF and known joint state; motion skipped")
                    return None
            ...
            if T is None:
                self.diag.warn(f"{where}: relative move without a known start position; motion skipped")
                return None
    ...
    def joint_goal(self, m: Motion) -> Optional[np.ndarray]:
        ...
        if isinstance(target, RelativeTarget) and target.joints is not None:
            if self.q_cur is None:
                self.diag.warn(f"{m.source}: relative joint move without a known start; motion skipped")
                return None
