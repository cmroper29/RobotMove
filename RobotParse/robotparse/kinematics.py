...
class RobotModel:
    def __init__(self, robot_cfg: dict, diag=None):
        ...
        m = self.model
        if m.nq != m.nv:
            raise ValueError("URDF contains continuous/planar/floating joints; only revolute and prismatic are supported")
        self.nq = m.nq
        ...
        if not m.existFrame(tip):
            raise ValueError(f"tip_link '{tip}' not found in URDF")
        self.tip_link = tip
    ...
    def to_urdf(self, q_ctrl) -> np.ndarray:
        ...
        if len(c) < self.nq:
            raise ValueError(f"joint target has {len(c)} values, robot has {self.nq} joints")
