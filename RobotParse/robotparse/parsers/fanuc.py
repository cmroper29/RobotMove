...
class FanucParser(BlockRunner):
    ...
    def _point(self, kind: str, idx_text: str, where: str) -> Optional[dict]:
        ...
        if kind.upper() == "PR":
            pr = self.pr.get(idx)
            if pr is None:
                self.diag.warn(f"{where}: PR[{idx}] has no value (set fanuc.position_registers in the config); motion skipped")
            return pr
        p = self.points.get(self.current_program, {}).get(idx)
        if p is None:
            self.diag.warn(f"{where}: P[{idx}] not found in /POS; motion skipped")
        return p
    ...
    def run_program(self, entry: Optional[str]) -> None:
        ...
        if name not in self.routines:
            raise ValueError(f"program '{name}' not loaded")
    ...
    def _motion(self, mtype: str, rest: str, s: Stmt) -> None:
        ...
        if not pm:
            self.diag.warn(f"{where}: unrecognised motion '{s.text[:60]}'")
            return
        ...
        if mtype == "C":
            ...
            if not pm2:
                self.diag.warn(f"{where}: circular motion without end point")
                return
            points.append((pm2.group("kind"), pm2.group("idx")))
            rest = pm2.group("rest")
        sm = SPEED_RE.match(rest)
        if not sm:
            self.diag.warn(f"{where}: could not read speed in '{s.text[:60]}'")
            return
